...
// 选择图片完毕->上传图片
const URL = ref('http://8.137.85.0:8008/files/download/img70ad74dc-ea79-11ee-8a0f-00163e04f5e4.png')
...
const onAddQues = async (text) => {
    ...
    loadingInstance1.close()
    console.log("下面是本次添加试题id")
    console.log(res.data.data.quesId)
    quesIdList.value.push(res.data.data.quesId)
    if (quesKindIdList.value.indexOf(res.data.data.quesKindId) == -1)
        quesKindIdList.value.push(res.data.data.quesKindId)
}
// 添加试卷
...
const onPaperComfirm = async () => {
    //上传多个坐标
    console.log("上传多个坐标的参数")
    console.log(newPoint.value)
    const res2 = await uploadPointsAPI({ structure: newPoint.value })
    console.log("坐标上传完毕，下面为返回结果")
    console.log(res2)

    // 添加试卷
    formModel.value.quesKindIds = '[' + quesKindIdList.value.join(',') + ']'
    formModel.value.quesIds = '[' + quesIdList.value.join(',') + ']'
    console.log(formModel.value.quesIds)
    const res = await addPaperAPI(formModel.value)
    console.log(res.data)
    ElMessage.success("添加试卷成功")
    console.log("下面是本次试卷id和试题id")
    console.log(res.data.data.paperId)
    console.log(res.data.data.quesIds)
    ...
    console.log("下面是更新切片的请求参数")
    console.log(data)
    const res1 = await updatePaperId(data)
...
<template>
    <div class="select" v-if="flag == 1">
        ...
        <!-- 请选择试卷图片 -->
        <div class="title">请选择试卷图片 <el-button type="primary" @click="onSelectEnd">选择完毕</el-button></div>
        <!-- 上传框 -->
        <div class="upload">
    ...
    <div class="selected">
        ...
        <div class="tool" v-if="flag == 2 || flag == 3">
            <el-button v-if="flag == 2" @click="onConfirm" type="success">请求图片切割</el-button>
            <el-button v-if="flag == 3" @click="onAddPaper" type="success">添加试卷</el-button>
        </div>
        <canvas></canvas>
    </div>
