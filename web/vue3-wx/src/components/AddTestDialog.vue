...
const getAllPaper = async () => {
  console.log("开始获取所有试卷")
  const res = await getAllPaperAPI()
  console.log(res.data)
  console.log("所有试卷获取成功!")
...
const onSubmit = async () => {
  ...
  const res = await addExamAPI(data1)
...
const onCancel = async () => {
  console.log("取消取消取消")
  dialogVisible.value = false
  const res = await addExamAfterAPI(examId.value)
}
</script>
<template>
  ...
  <el-dialog v-model="centerDialogVisible" title="选择考试试卷" width="500" center>
    ...
    <el-button @click="onPaperConfirm" type="primary" class="btn">确定选择</el-button>
